...
const SALONS = ["ANA SALON 1", "ANA SALON 2", "ANA SALON 3", "ANTRENMAN 1", "ANTRENMAN 2", "ANTRENMAN 3", "ANTRENMAN 4", "ANTRENMAN 5"];
...
function IconBall(props) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" {...props}>
      <circle cx="12" cy="12" r="9.2" />
      <path d="M12 2.8c3.2 2.3 5.1 5.8 5.1 9.2s-1.9 6.9-5.1 9.2c-3.2-2.3-5.1-5.8-5.1-9.2S8.8 5.1 12 2.8Z" />
      <path d="M3 9.6c2.4 1.6 5.7 2.5 9 2.5s6.6-.9 9-2.5" />
      <path d="M3 14.4c2.4-1.6 5.7-2.5 9-2.5s6.6.9 9 2.5" />
    </svg>
  );
}
...
function NetDivider({ className = "" }) {
  return (
    <div className={`relative flex items-center ${className}`}>
      <div className="net-divider flex-1" />
      <IconBall className="w-3.5 h-3.5 mx-3 text-court-500 shrink-0" />
      <div className="net-divider flex-1" />
    </div>
  );
}
...
export default function Home() {
  ...
  const [activeGroup, setActiveGroup] = useState(Object.keys(INITIAL_DATA)[0]);
  ...
  const handleScoreChange = (group, matchId, field, value) => {
    setData(prev => {
      const updatedMatches = prev[group].matches.map(m => {
        if (m.id === matchId) {
          return { ...m, [field]: value === "" ? "" : Math.max(0, parseInt(value, 10) || 0) };
        }
        return m;
      });
  ...
  const currentTable = useMemo(() => calculateTable(activeGroup), [data, activeGroup]);
  ...
  // Global scoreboard stats across every group
  const globalStats = useMemo(() => {
    let total = 0, done = 0;
    Object.values(data).forEach(g => {
      g.matches.forEach(m => {
        total += 1;
        if (m.homeScore !== "" && m.awayScore !== "") done += 1;
      });
    });
  ...
  const groupProgress = (groupName) => {
    const g = data[groupName];
    const total = g.matches.length;
    const done = g.matches.filter(m => m.homeScore !== "" && m.awayScore !== "").length;
    return { total, done, pct: total ? Math.round((done / total) * 100) : 0 };
  };

  return (
    <div className="min-h-screen bg-court-950 text-[#eef7f4] font-body selection:bg-flame-500 selection:text-white">
      <Head>
        <title>Eczacıbaşı Geleceğe Smaç Mini Voleybol Şenliği</title>
        <meta name="description" content="Geleceğe Smaç Mini Voleybol Şenliği canlı skor ve puan durumu takip paneli" />
      </Head>

      {/* HEADER */}
      <header className="bg-court-900/95 backdrop-blur border-b border-court-700 sticky top-0 z-50">
        <div className="max-w-[1400px] mx-auto px-5 md:px-8 pt-5 pb-4 flex flex-col md:flex-row justify-between items-center gap-5">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-gradient-to-br from-flame-500 to-flame-600 rounded-2xl shadow-lg shadow-flame-500/30 flex items-center justify-center shrink-0">
              <IconBall className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-lg md:text-xl font-display font-bold tracking-tight text-white uppercase leading-tight">
                Eczacıbaşı Geleceğe Smaç
              </h1>
              <p className="text-[11px] text-flame-400 font-mono font-bold tracking-[0.18em] uppercase mt-0.5">
                Mini Voleybol Şenliği · Canlı Takip Paneli
              </p>
            </div>
          </div>

          <div className="w-full md:w-auto relative">
            <IconSearch className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-court-400" />
            <input
              type="text"
              placeholder="Takım ara... (örn. A1)"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full md:w-72 bg-court-850 border border-court-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white focus:outline-none focus:border-flame-500 focus:ring-2 focus:ring-flame-500/20 transition-all placeholder:text-court-400"
            />
          </div>
        </div>

        {/* SCOREBOARD STRIP */}
        <div className="max-w-[1400px] mx-auto px-5 md:px-8 pb-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-court-850 border border-court-700 rounded-2xl px-4 py-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-court-400">Toplam Maç</p>
              <p className="font-mono text-2xl font-bold text-white mt-1">{globalStats.total}</p>
            </div>
            <div className="bg-court-850 border border-court-700 rounded-2xl px-4 py-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-court-400">Tamamlanan</p>
              <p className="font-mono text-2xl font-bold text-emerald-400 mt-1">{globalStats.done}</p>
            </div>
            <div className="bg-court-850 border border-court-700 rounded-2xl px-4 py-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-court-400">Kalan Maç</p>
              <p className="font-mono text-2xl font-bold text-ball-400 mt-1">{globalStats.remaining}</p>
            </div>
            <div className="bg-court-850 border border-flame-500/40 rounded-2xl px-4 py-3 relative overflow-hidden">
              <p className="text-[10px] font-bold uppercase tracking-widest text-court-400">İlerleme</p>
              <p className="font-mono text-2xl font-bold text-flame-400 mt-1">%{globalStats.pct}</p>
              <div className="absolute bottom-0 left-0 h-1 bg-court-700 w-full">
                <div className="h-full bg-gradient-to-r from-flame-600 to-flame-400 transition-all duration-700" style={{ width: `${globalStats.pct}%` }} />
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-[1400px] mx-auto px-5 md:px-8 py-8 md:py-10">

        {/* GROUP SELECTOR — COURT TABS */}
        <div className="mb-6">
            ...
            {Object.keys(data).map(group => {
              ...
              return (
                <button
                  ...
                  className={`group relative flex flex-col items-start px-5 py-3 rounded-2xl border transition-all duration-300 min-w-[180px] ${
                    isActive
                      ? 'bg-gradient-to-br from-flame-600 to-flame-500 border-flame-400 shadow-lg shadow-flame-500/25'
                      : 'bg-court-850 border-court-700 hover:border-court-500'
                  }`}
                >
                  <span className={`text-xs font-display font-bold uppercase tracking-wide ${isActive ? 'text-white' : 'text-[#eef7f4]'}`}>
                    {group}
                  </span>
                  <span className={`text-[10px] font-mono mt-1 ${isActive ? 'text-white/80' : 'text-court-400'}`}>
                    {prog.done}/{prog.total} maç tamamlandı
                  </span>
                  <div className={`mt-2 h-1 w-full rounded-full overflow-hidden ${isActive ? 'bg-white/25' : 'bg-court-700'}`}>
                    <div
                      className={`h-full rounded-full ${isActive ? 'bg-white' : 'bg-flame-500'}`}
            ...
            <button
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wide transition-all ${
                activeRoundFilter === "Tümü"
                  ? 'bg-white text-court-950'
                  : 'bg-court-850 text-court-300 border border-court-700 hover:border-court-500 hover:text-white'
              }`}
            >
              Tüm Fikstür
            </button>
              ...
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wide transition-all ${
                  activeRoundFilter === round
                    ? 'bg-white text-court-950'
                    : 'bg-court-850 text-court-300 border border-court-700 hover:border-court-500 hover:text-flame-400'
                }`}
              >
                {round.split('(')[0].trim()}
              </button>
        ...
        <NetDivider className="mb-8" />

        {/* MAIN GRID */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">

          {/* SCORE ENTRY PANEL */}
          <div className="lg:col-span-5 bg-court-900 border border-court-700 rounded-3xl p-5 md:p-7">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-sm font-display font-bold text-white uppercase tracking-wide">Skor Girişi</h2>
                <p className="text-xs text-court-400 mt-1">{activeRoundFilter === "Tümü" ? "Tüm maçlar" : activeRoundFilter}</p>
              </div>
              <span className="bg-court-800 text-flame-400 font-mono font-bold text-[11px] px-3 py-1.5 rounded-lg border border-court-700">
                {displayedMatches.length} maç
              </span>
            </div>

            <div className="space-y-3 max-h-[640px] overflow-y-auto pr-2 court-scrollbar">
              {displayedMatches.length === 0 ? (
                <div className="text-center py-16 text-court-400 font-medium text-sm bg-court-850 rounded-2xl border border-dashed border-court-700">
                ...
                displayedMatches.map(match => {
                  ...
                  return (
                    <div
                      key={match.id}
                      className={`animate-fadeUp flex flex-col p-4 rounded-2xl border transition-all duration-300 ${
                        hasScore
                          ? 'bg-court-850/60 border-court-700'
                          : 'bg-court-850 border-court-600 hover:border-flame-500/60 hover:shadow-lg hover:shadow-flame-500/5'
                      }`}
                    >
                      <div className="flex justify-between items-center pb-3 mb-3 border-b border-court-700/60">
                        <span className={`text-[10px] font-mono font-bold uppercase tracking-widest px-2.5 py-1 rounded-md ${
                          hasScore ? 'bg-court-800 text-court-400' : 'bg-flame-500/10 text-flame-400'
                        }`}>
                          {match.round.split('(')[0].trim()}
                        </span>
                        <span className="text-[10px] font-bold text-court-400 uppercase tracking-wider flex items-center gap-1.5">
                      ...
                      <div className="flex items-center justify-between gap-2 px-1">
                        <span className={`w-[30%] text-right font-display font-bold text-sm truncate ${homeWins ? 'text-white' : 'text-[#cfe3e5]'}`}>
                          {match.home}
                        </span>
                        ...
                        <div className="flex items-center gap-2.5 shrink-0">
                          <input
                            ...
                            className="score-input w-12 h-11 bg-court-950 text-center rounded-xl border border-court-600 font-mono font-bold text-lg text-flame-400 focus:outline-none focus:border-flame-500 focus:bg-court-900 transition-all placeholder:text-court-600"
                          />
                          <span className="text-court-500 font-mono font-bold text-xs">:</span>
                          <input
                            type="number"
                            inputMode="numeric"
                            pattern="[0-9]*"
                        ...
                        <span className={`w-[30%] text-left font-display font-bold text-sm truncate ${awayWins ? 'text-white' : 'text-[#cfe3e5]'}`}>
                          {match.away}
                        </span>
                      </div>
                    </div>
          ...
          {/* STANDINGS PANEL */}
          <div className="lg:col-span-7 bg-court-900 border border-court-700 rounded-3xl p-5 md:p-7">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-sm font-display font-bold text-white uppercase tracking-wide flex items-center gap-2">
                  <IconTrophy className="w-4 h-4 text-ball-400" />
                  Puan Durumu
                </h2>
                <p className="text-xs text-court-400 mt-1">Sıralama anlık olarak güncellenir</p>
              </div>
            </div>

            <div className="overflow-x-auto rounded-2xl border border-court-700 bg-court-950">
              <table className="w-full text-left text-sm table-fixed min-w-[600px]">
                <thead className="bg-court-850 font-bold uppercase text-court-400 text-[10px] tracking-wider border-b border-court-700">
                  <tr>
                    <th className="w-12 py-4 text-center">Sıra</th>
                    <th className="w-36 px-4 py-4">Takım</th>
                    ...
                    <th className="w-10 py-4 text-center text-court-500" title="Atılan Sayı">AS</th>
                    <th className="w-10 py-4 text-center text-court-500" title="Yenilen Sayı">YS</th>
                    <th className="w-10 py-4 text-center text-court-500" title="Averaj">AV</th>
                    <th className="w-16 py-4 text-center text-flame-400 bg-flame-500/10">PTS</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-court-700/60 font-semibold text-[#dceaec]">
                  {currentTable
                    .filter(row => !searchTerm || row.name.toLowerCase().includes(searchTerm.toLowerCase()))
                    .map((row, idx) => {
                      const medal = idx === 0 ? 'bg-ball-500 text-court-950 shadow-md shadow-ball-500/40'
                        : idx === 1 ? 'bg-[#c7d2d6] text-court-950'
                        : idx === 2 ? 'bg-[#d99a5c] text-court-950'
                        : null;
                      const rowAccent = idx === 0 ? 'border-l-2 border-l-ball-500 bg-ball-500/[0.04]' : '';
                      return (
                        <tr key={row.name} className={`hover:bg-court-850 transition-colors ${rowAccent}`}>
                          <td className="py-3.5 text-center">
                            {medal ? (
                              <span className={`w-6 h-6 flex items-center justify-center rounded-full mx-auto text-xs font-mono font-bold ${medal}`}>{idx + 1}</span>
                            ) : (
                              <span className="text-court-500 font-mono">{idx + 1}</span>
                            )}
                          </td>
                          <td className={`px-4 py-3.5 truncate font-display ${idx === 0 ? 'text-white' : 'text-[#dceaec]'}`}>
                            {row.name}
                          </td>
                          <td className="py-3.5 text-center text-court-400 font-mono">{row.gp}</td>
                          <td className="py-3.5 text-center text-white font-mono">{row.w}</td>
                          <td className="py-3.5 text-center text-white font-mono">{row.d}</td>
                          <td className="py-3.5 text-center text-white font-mono">{row.l}</td>
                          <td className="py-3.5 text-center text-court-500 font-mono">{row.gf}</td>
                          <td className="py-3.5 text-center text-court-500 font-mono">{row.ga}</td>
                          <td className={`py-3.5 text-center font-mono ${row.gd > 0 ? 'text-emerald-400' : row.gd < 0 ? 'text-rose-400' : 'text-court-500'}`}>
                            {row.gd > 0 ? `+${row.gd}` : row.gd}
                          </td>
                          <td className="py-3.5 text-center font-mono font-bold text-flame-400 bg-flame-500/5 text-base">
                            {row.pts}
                          </td>
                        </tr>
                      );
            ...
            <p className="text-[10px] text-court-500 mt-4 font-mono uppercase tracking-wide">
              O: Oynanan · G: Galibiyet · B: Beraberlik · M: Mağlubiyet · AS/YS: Atılan/Yenilen Sayı · AV: Averaj
            </p>
          </div>
      ...
      <footer className="border-t border-court-700 mt-10">
        <div className="max-w-[1400px] mx-auto px-5 md:px-8 py-6 text-center text-[11px] text-court-500 font-mono uppercase tracking-widest">
          {SALONS.length} aktif salon · {Object.keys(data).length} grup · Eczacıbaşı Geleceğe Smaç Mini Voleybol Şenliği
        </div>
      </footer>
    </div>
  );
